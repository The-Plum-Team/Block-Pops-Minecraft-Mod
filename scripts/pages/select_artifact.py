...
class GitHubApi:
    ...
    def runs(self, workflow_id: int, branch: str) -> list[dict[str, Any]]:
        ...
        value = self.get(f"/repos/{self.repository}/actions/workflows/{workflow_id}/runs?{query}")
    ...
    def artifacts_for_run(self, run_id: int) -> list[Artifact]:
        ...
        return [Artifact.parse(row) for row in rows]
...
def _run_order(run: dict[str, Any]) -> tuple[datetime, int]:
...
def select(
...
) -> tuple[str, Artifact, int, int, int]:
    ...
    for run in api.runs(e2e_workflow["id"], branch):
        try:
            ...
            _validate_run(
                ...
                repository=repository, branch=branch, sha=commit,
    ...
    artifacts = [
        ...
        and artifact.run_id == source_run_id and artifact.head_branch == branch
        and artifact.head_sha == commit and 0 < artifact.size <= MAX_ARTIFACT_BYTES
    ]
    if len(artifacts) > 1:
        raise SelectionError("successful exact-head E2E run owns duplicate raw handoffs")
    if artifacts:
        return "raw", artifacts[0], attempt, source_run_id, attempt

    cache_name = cache_artifact_name(branch, commit)
    pages_workflow = api.workflow("pages.yml")
    cache_candidates: list[Artifact] = []
    for artifact in api.artifacts_named(cache_name):
        ...
        try:
            ...
            continue
        cache_candidates.append(artifact)
    ...
    return (
        "compact",
        max(cache_candidates, key=lambda artifact: artifact.order),
        0,
        source_run_id,
        attempt,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        kind, artifact, run_attempt, source_run_id, source_run_attempt = select(
