...
@contextmanager
def _cache_input(context, *, root, branch, repository, byte_budget=site.MAX_SITE_BYTES):
    """Keep one validated compact's descriptors/stamps live for its enclosing action."""
    if type(byte_budget) is not int or not 0 <= byte_budget <= site.MAX_SITE_BYTES:
        raise site.SiteError("cache byte budget is invalid")
    with ExitStack() as handles:
        rows = [row for row in context["rows"] if row["name"] == branch]
        if len(rows) != 1:
        ...
        expected = dict(repository=repository, branch=branch,
        ...
        matrix_path = context["branches"][branch][0]
        payloads = {"manifest.json": manifest_raw, "release-matrix.json": matrix_path.read_bytes()}
        if sum(map(len, payloads.values())) + sum(record["size"] for record in manifest["files"]) > byte_budget:
            raise site.SiteError("caches exceed the aggregate byte budget")
        ...
        def recheck():
            if not live:
                raise site.SiteError("cache input lease is closed")
            final[0]()
            site.atomic._bound_output_directory(root, parent, root.name, descriptor)
        recheck()
        try:
            yield manifest, count, size, recheck
        finally:
            live = False
...
def validate_current_cache(api, args):
    ...
    with site._current_pages_inputs(api, args, phase="refresh") as context, _cache_input(
            context, root=args.input, branch=args.branch, repository=args.repository) as cached:
        manifest, count, size, recheck = cached
        context["recheck"]()
        recheck()
        row = next(row for row in context["rows"] if row["name"] == args.branch)
