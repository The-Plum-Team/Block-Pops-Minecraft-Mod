"""Opt-in complete gifting compilation with native ItemStack/codec checks, without HTTP."""
...
from tests.test_block_entity_item_data import PROBE

ROOT = Path(__file__).resolve().parents[1]
PACKAGE = Path("common/src/main/java/com/theplumteam")
CONSUMERS = ("network/DropBoxPacket", "network/UnlockCollectionPacket", "command/GetBoxCommand")
HELPERS = ("item/BlockEntityItemData", "util/AuthlibProfiles", "util/ResourceLocations",
           "network/PacketNetworking", "client/ClientPacketNetworking")
CLASSES = (*CONSUMERS, *HELPERS)
...
REQUIRED = ("BLOCKPOPS_TEST_GRADLE", "BLOCKPOPS_TEST_GRADLE_HOME", "BLOCKPOPS_TEST_JAVA17",
    ...
    "BLOCKPOPS_TEST_DECLARATIONS", "BLOCKPOPS_TEST_GIFTING_BASELINES")
OLD_WRITE = 'boxItem.getOrCreateTag().put("BlockEntityTag", blockEntityTag);'
NEW_WRITE = 'BlockEntityItemData.write(boxItem, blockEntityTag, "blockpops:box_block");'


@unittest.skipUnless(all(os.environ.get(key) for key in REQUIRED), "explicit Gradle/JDK/API/baseline inputs required")
class GiftingItemDataApiTests(unittest.TestCase):
    def test_complete_consumers_and_current_helpers_preserve_item_data(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            baselines = {name: Path(path).resolve() for name, path in
                         json.loads(os.environ["BLOCKPOPS_TEST_GIFTING_BASELINES"]).items()}
            self.assertEqual(set(CONSUMERS), set(baselines))
            ...
            for path, raw in sources.items():
                ...
                name = path.relative_to(PACKAGE).as_posix().removesuffix(".java")
                if name in baselines:
                    newline = b"\r\n" if b"\r\n" in raw else b"\n"
                    added = b"import com.theplumteam.item.BlockEntityItemData;" + newline
                    self.assertEqual(1, raw.count(added)); self.assertEqual(1, raw.count(NEW_WRITE.encode()))
                    reverted = raw.replace(added, b"", 1).replace(NEW_WRITE.encode(), OLD_WRITE.encode(), 1)
                    self.assertEqual(baselines[name].read_bytes(), reverted)
...
rootProject.name = 'gifting-item-data-api-probe'
...
""")
            ...
            for version, major in (("1.20.1", 17), ("1.21.1", 21)):
                ...
                for name, path, source in zip(CLASSES, generated, SOURCES, strict=True):
                ...
                for name in CLASSES:
                    ...
                    if name not in CONSUMERS: continue
                    ...
                    self.assertEqual(0, code, output)
                    self.assertEqual(1, output.count("BlockEntityItemData.write:"))
                    self.assertIn("String blockpops:box_block", output)
                    self.assertNotIn("getOrCreateTag:", output)
                    self.assertEqual(1, output.count("AuthlibProfiles.fetch:"))
                    self.assertEqual(1, output.count("AuthlibProfiles.value:"))
                if major == 17:
                    code, output = invoke("legacy-add-tag-bytecode", [homes[17] / "bin/javap", "-p", "-c",
                        "-classpath", cp, "net.minecraft.world.item.ItemStack"])
                    self.assertEqual(0, code, output)
                    method = output.split("public void addTagElement(", 1)[1].split("\n  public ", 1)[0]
                    self.assertRegex(method, r"(?s)getOrCreateTag:.*CompoundTag\.put:.*\bpop\b.*\breturn\b")
                # Only vanilla ItemStack and the current helper execute, never baseline symbols or gifting/gameplay.
                runtime_cp = os.pathsep.join(map(str, [destination, *classpaths[version]]))
                code, output = invoke(f"native-item-data-{version}", [homes[major] / "bin/java", "-Djava.awt.headless=true",
                    "-cp", runtime_cp, "DataProbe"])
            ...
            (logs / "result.json").write_text(json.dumps({"kind": "gifting-item-data-api-probe", "qualified": False,
                ...
                "observations": observations, "commands": commands, "complete_consumers_and_current_helpers_compiled": True,
                "baseline_symbols_executed": False, "consumer_bodies_unchanged_except_item_write": True,
                "http_or_gameplay_executed": False, "vanilla_bootstrap": True}, indent=2) + "\n")
