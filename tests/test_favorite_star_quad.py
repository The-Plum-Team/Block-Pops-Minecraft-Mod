...
CLASSES = ("client/gui/FavoriteColorSelectionScreen", "client/gui/widget/ColorSelectionButton",
    "item/BlockEntityItemData", "block/PopBlockColor", "network/SetFavoriteColorPacket",
    "network/PacketNetworking", "client/ClientPacketNetworking", "util/ResourceLocations")
...
REQUIRED = ("BLOCKPOPS_TEST_GRADLE", "BLOCKPOPS_TEST_GRADLE_HOME", "BLOCKPOPS_TEST_JAVA17",
    ...
    "BLOCKPOPS_TEST_FAVORITE_BASELINE", "BLOCKPOPS_TEST_FAVORITE_HISTORICAL")


def modern_quad(source):
    start = source.index("BufferBuilder bufferBuilder = Tesselator.getInstance().begin(")
...
class FavoriteStarQuadApiTests(unittest.TestCase):
    def test_complete_screen_preserves_quad_geometry_and_legacy_sources(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            baseline = Path(os.environ["BLOCKPOPS_TEST_FAVORITE_BASELINE"]).resolve()
            historical = [Path(path).resolve() for path in json.loads(os.environ["BLOCKPOPS_TEST_FAVORITE_HISTORICAL"])]
...
rootProject.name = 'favorite-star-quad-api-probe'
...
""")
            ...
            for version, major in (("1.20.1", 17), ("1.21.1", 21)):
                ...
                code, output = invoke(f"screen-bytecode-{version}", [homes[major] / "bin/javap", "-p", "-c", "-classpath",
                    destination, "com.theplumteam.client.gui.FavoriteColorSelectionScreen"])
                ...
                if major == 21:
                    code, output = invoke("reject-legacy-api-on-modern", [homes[21] / "bin/javac", "-proc:none", "--release", "21",
                        ...
                        root / "common/versions/1.20.1/build/generated/stonecutter/main/java/com/theplumteam/client/gui/FavoriteColorSelectionScreen.java"])
                    self.assertNotEqual(0, code); self.assertIn("method getBuilder()", output)
                    self.assertIn("method end()", output); self.assertIn("7 errors", output)
            ...
            (logs / "result.json").write_text(json.dumps({"kind": "favorite-star-quad-api-probe", "qualified": False,
