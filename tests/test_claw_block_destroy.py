"""Opt-in complete Claw compilation against native APIs; no gameplay or mocked API."""
...
CLASSES = ("block/ClawMachineBlock", "blockentity/ClawMachineBlockEntity", "block/BlockInteractionResults")
...
REQUIRED = ("BLOCKPOPS_TEST_GRADLE", "BLOCKPOPS_TEST_GRADLE_HOME", "BLOCKPOPS_TEST_JAVA17",
    ...
    "BLOCKPOPS_TEST_DECLARATIONS", "BLOCKPOPS_TEST_LEGACY_CLAW_BLOCK")


@unittest.skipUnless(all(os.environ.get(key) for key in REQUIRED), "explicit Gradle/JDK/API/baseline inputs required")
class ClawBlockDestroyApiTests(unittest.TestCase):
    def test_complete_claw_preserves_drops_and_returns_the_native_super_result(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            baseline = Path(os.environ["BLOCKPOPS_TEST_LEGACY_CLAW_BLOCK"]).resolve()
...
rootProject.name = 'claw-block-destroy-api-probe'
...
""")
            ...
            for version, major in (("1.20.1", 17), ("1.21.1", 21)):
                ...
                if major == 17:
                    self.assertEqual(java_tokens(baseline.read_text()), java_tokens(generated[0].read_text()))
                ...
                code, output = invoke(f"bytecode-{version}", [homes[major] / "bin/javap", "-p", "-c",
                    "-classpath", destination, "com.theplumteam.block.ClawMachineBlock"])
                self.assertEqual(0, code, output)
                destroy = output.split(" playerWillDestroy(", 1)[1].split(" onRemove(", 1)[0]
                self.assertLess(destroy.index("Player.isCreative:"), destroy.index("preventCreativeDropFromBottomPart:"))
                self.assertLess(destroy.index("preventCreativeDropFromBottomPart:"), destroy.index("dropResources:"))
                self.assertLess(destroy.index("dropResources:"), destroy.index("BaseEntityBlock.playerWillDestroy:"))
                self.assertRegex(destroy, r"BaseEntityBlock.playerWillDestroy:[^\n]+\n\s*\d+: " + ("areturn" if major == 21 else "return"))
                ...
                if major == 21:
                    rejected = root / "rejected/ClawMachineBlock.java"; rejected.parent.mkdir()
                    rejected.write_text(generated[0].read_text().replace("public BlockState playerWillDestroy(",
                        "public void playerWillDestroy(").replace("return super.playerWillDestroy(", "super.playerWillDestroy("))
                    code, output = invoke("old-destroy-api-rejected-by-modern", [homes[21] / "bin/javac", "-proc:none",
                        "--release", "21", "-cp", str(destination) + os.pathsep + cp, "-d", root / "rejected-classes", rejected])
                    self.assertNotEqual(0, code)
                    self.assertIn("return type void is not compatible with BlockState", output)
            ...
            (logs / "result.json").write_text(json.dumps({"kind": "claw-block-destroy-api-probe", "qualified": False,
                ...
                "legacy_active_claw_block_identical": True, "world_or_gameplay_executed": False}, indent=2) + "\n")
