...
| AI boundary | `.github/workflows/visual-review.yml`, protected provider adapter | Reuse secretless authentication/curation, bounded single-use capsule, fresh `visual-review` environment runner, fixed no-tools Responses API client, protected normalization, and exact artifact-ID cleanup. AI remains advisory. |
