...
- **1.21.10 and 1.21.11** — Architectury Loom 1.11 cannot consume the Fabric API of this era: configuring `:common:1.21.11` fails with "Javadoc provided by mod (fabric-content-registries-v0) must have an intermediary source namespace". The sibling Quick-Skin mod reaches these versions on Loom 1.17.480 and Gradle 9.7.1, where this repository is on Loom 1.11.456 and Gradle 8.14. Moving those is a build migration that would touch every lane, the dependency verification inventory, the isolated frontier harness and the release pipeline's recorded digests; it is its own change, not a version port. 1.21.10's only GeckoLib is an alpha besides.
- **26.1 through 26.3** — GeckoLib publishes these to Modrinth and CurseForge only. Nothing is on its Maven repository, nothing is on Central, and Modrinth's Maven keys artifacts by project version alone, which cannot distinguish the Fabric 26.2 build from the NeoForge 26.3 build that share a version number. They also need Java 25 and the same Loom and Gradle move.
