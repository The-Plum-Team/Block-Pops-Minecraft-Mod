...
## Task 10s — retain the link button's normal native background

- LinkButton now selects the vanilla widget/button sprite on modern Minecraft; the legacy nine-slice statement and all active Java tokens stay unchanged. Native AbstractButton bytecode and the shipped sprite/scaling metadata confirm the normal-state resource and API. Historical6b5651e6/4f6e4039 instead changed hover, fill and border behavior; those bodies were not imported. Logo rendering, click handling and shader state remain unchanged.
- Root passes strict/offline Stonecutter and complete LinkButton compilation on javac17/21 (majors61/65), verifies exact legacy tokens and reproduces the old WIDGETS_LOCATION failure against the modern API. Independent source review finds no blocker. Evidence is build/diagnostics/link-button/{closure.json,probe/result.json}; one baseline BlockPopsMod declaration is compile-only and all input hashes remain unchanged. This eight-line API guard has a local diagnostic probe; no actual GUI rendering or gameplay executes. Rollback is these guards.

## Task 10t — preserve color-widget item state through copied components

- ColorSelectionButton reads existing block-entity data, creates it only when absent, and writes after each constructor/toggle mutation. Player-null behavior, explicit empty figure IDs, offsets, colors, unrelated fields and legacy aliases remain intact. Historical6b5651e6/4f6e4039 recreated tags on each toggle and omitted the required component ID; those bodies were not adopted. Reversing the bounded substitutions restores the entire original widget byte for byte.
- Root repeats strict/offline Stonecutter and complete widget/helper/enum compilation on javac17/21 in 9.296 seconds. Extracted actual mutation bodies execute against native ItemStack/CompoundTag APIs and codecs for all colors, nullable players, hide/show, cached tags, extra nested data and absent/wrong legacy tag types. All 196 inputs and complete class hashes match the implementation run; 106 baseline declarations are compile-only. Evidence is build/diagnostics/color-selection-data/{closure,root-review}.json and root-probe/. No client, GUI, HTTP or gameplay executes. Rollback is this widget adapter and opt-in probe.

## Task 10u — preserve renderer color forwarding across GeckoLib signatures

- BoxBlockRenderer and FigureBlockRenderer select the modern packed colour parameter and forward it unchanged; the three fixed-white face/logo calls and FigureEntry use opaque ARGB white. Their complete modern renderer tokens match both historical6b5651e6/4f6e4039; FigureEntry retains its current logging/error policy. All three legacy active sources remain identical.
- Root passes strict/offline Stonecutter and complete javac17/21 compilation of the three consumers plus seven current dependencies, including entities, item-data/identifier helpers, skin detector, box model and widget renderer. Bytecode verifies all native color signatures and white constants; independent source review finds no blocker. The 185 inputs remain unchanged, including96 compile-only baseline declarations; the two figure models still use declarations because asynchronous skin registration remains pending. Evidence is build/diagnostics/gecko-render-colour/{closure.json,probe/result.json}. No baseline class, renderer, GL or gameplay executes. Rollback is the56 guarded source lines; the reproducible local diagnostic is retained.

## Task 12aj — validate deployed branch caches within authenticated companion leases

- The shared Pages input context retains all descriptor leases and final API/source/companion seals through its caller. The new refresh_cache command validates one enrolled branch after independently observing exact successful Build and Deploy jobs in the same in-progress attempt, then rechecks those prerequisites and seals compact bytes, inventory and directory identity. Branch matrix/selection determine scope and projection. Its summary carries no authority and writes no cache; the caller must supply the collected bundle through authenticated current-run transport.
- Root repeats all45 refresh/companion/site/transport tests in90.571 seconds and independent review finds no blocker. Real Git/ZIP/pixels cover preparing/full/mixed branches, prerequisite failures before download, crossed selections, late job/source/payload/directory drift and live leases through final rechecks. Existing build/default behavior passes unchanged. No workflow wiring or remote action is included; refresh workflow and rotation adapters remain pending. Rollback is the shared context, validation CLI and matching tests.

## Task 12ak — wire branch cache validation after successful deployment

- The refresh job retains the exact legacy validation command for schema1 and selects the authenticated refresh CLI for a scoped protected matrix. It passes the branch plus literal discovery inventory outside the checkout, protected implementation/default and exact run/attempt, with explicit shell guards and token isolation. The existing current-run download remains bound to the exact attempt/manifest by the separately authenticated companion. Download/upload paths and permissions are unchanged.
- Root repeats all53 focused shell/workflow/security/graph tests in6.416 seconds and independent review finds no blocker. Helper, invocation and validator failures stop before promotion; quoted values cannot execute shell content and payload bytes remain unchanged. Only this validation step and its shell tests change. Discovery and rotation remain pending; no workflow or remote action was run. Rollback is the step and tests together.

## Task 10v — retain the favorite-color screen's star quad across vertex APIs

- FavoriteColorSelectionScreen selects modern begin/addVertex/setUv/buildOrThrow/drawWithShader while retaining all positions, UVs, shader state and surrounding behavior. The complete legacy tokens remain identical; modern quad tokens match both historical6b5651e6/4f6e4039 without importing unrelated screen changes.
- Root repeats strict/offline Stonecutter and complete screen plus seven current dependency compilations on javac17/21 in5.499 seconds. All193 inputs and complete class hashes match the implementation run. A negative fixture restores the old quad and reproduces all seven original modern API errors. The101 baseline declarations remain compile-only. Evidence is build/diagnostics/favorite-star-quad/{closure,root-review}.json and root-probe/. No GL, client, HTTP or gameplay executes. Rollback is the12 guarded source lines and opt-in probe.

## Task 10w — retain the original local profile properties across client construction

- A client-only Minecraft mixin captures the exact GameConfig.user.profileProperties reference per instance. Both native eras receive that initial map; modern Minecraft otherwise discards it and exposes a separately fetched secure profile. This capture makes the original local fallback source available without waiting for that future, fetching anything or changing legacy profile handling. Scheduling and registration callers remain a separate seam.
- Root repeats the actual Fabric Knot/Mixin transformation on Minecraft1.20.1/1.21.1 with matrix-pinned loaders and JDK17/21 in4.924 seconds. ASM proves capture at constructor returns and an instance getter without copying; Minecraft remains uninitialized. Separate real-type checks preserve populated/empty maps, aliases and instance isolation, while a required missing-target injection fails in both eras. All108 inputs match; generated Mixin class hashes are recorded separately rather than assumed reproducible. Evidence is build/diagnostics/local-profile-properties/{closure,root-review}.json and root-probe/.
- Local Loom bytecode confirms existing automatic AP/refmap handling; no build/bootstrap paths change. Remapped production JAR inspection and Forge loading are not established by this named Fabric probe and remain explicit follow-up checks. No client launch, rendering, network or gameplay occurs. Rollback is the two Java files, client config entry and opt-in test together.

## Integrated legacy checkpoint — clean local commit 3c92a67

- A separate no-remote checkout of3c92a67e3d9c4e80ea6ca3ee609d58ee231ca7c6, tree48b9897ecd0fa0329139f886b29cf5d129c9d37c, passes strict serial production/harness/check for Fabric1.20.1 in44 seconds and Forge1.20.1 in38 seconds. The runner exits0 in99.233 seconds, both staging checks exit0, and all756 source inputs remain identical and clean. Private APFS-cloned caches have distinct inodes; prior evidence remains unchanged and owned processes close.
- Root verifies staged identities, production/harness boundaries, class majors and report bindings. Both production JARs contain the required client mixin/interface, referenced JSON refmap, correct loader metadata and exact Minecraft/GameConfig field names remapped through the real Fabric/Forge mappings. This closes the packaging follow-up from task10w; runtime Forge transformation and gameplay remain untested. Evidence is build/diagnostics/task10-profile-legacy/{root-review,artifact-audit}.json and production-inspection/. Report SHA256 cec90c66b6e169c63c3015b4730b1453cf8a715806dbf32fc2dcd707ffe42f07; manifest SHA256 a2b99160d26e32d1d73d6ae7ab670d04456880bc312d9ee2930b0e6f9250db14. Scope remains explicitly partial2/12, without publication or lane qualification.

## Task 12al — retain authenticated current-attempt caches for future rotation actions

- A new rotation input context requires exact successful collect/build/deploy/promotion coverage in the same current attempt, preserves every companion/cache descriptor and enforces aggregate byte/file limits across branches. Rechecks join live API/source observations with all cache seals and the exact root inventory/path binding; callbacks reject use after closure. Refresh reuses the extracted cache reader without changing its command behavior.
- Root repeats all54 input/refresh/site/transport/legacy-plan tests in135.446 seconds; independent review finds no blocker. Real Git/ZIP/pixels cover mixed scopes, missing/duplicate/cross-attempt promotion jobs, cache substitutions, earlier-cache mutations during later hashing, aggregate quotas and post-yield drift. Existing rotation API, planner, invocation validator and CLI bodies remain identical. No plan, DELETE, workflow or remote action is enabled by this lease alone; a later caller must retain it and recheck before each action. Completed-owner rotation remains separate. Rollback is the lease, shared cache reader and tests.

## Task 10x — retain independent list bounds for scaled native operations

- BoundedSelectionList forwards the legacy constructor unchanged. Its modern branch retains independent edges and scrolling limits, maps the existing background/separator controls, and temporarily supplies viewport height to native render/drag/centering with finally restoration. This preserves separate rounded edges and stored dimensions instead of losing a pixel through width/height reconstruction. Existing widgets are not connected yet; arbitrary modern setX/setY/updateSize usage is outside this bounded adapter contract.
- Root repeats strict/offline preprocessing, javac17/21 and native list operations in5.188 seconds, with all87 inputs, class hashes and complete native outputs matching. Fractional scales, inclusive bounds, row geometry, clamp/center/visibility/drag and normal/exception restoration pass; render stops at a sentinel before GL and disabled decorations need no graphics context. Two initial fixture failures (missing annotation API and abstract entry narration) are retained and corrected without product changes. Evidence is build/diagnostics/bounded-selection-list/{closure,root-review}.json and root-probe/. No client/world/gameplay runs; vanilla scrollbar-edge hover differences remain for interaction qualification. Rollback is the base and opt-in probe.

## Integrated regression checkpoint — clean local commit 3c92a67

- The same separate no-remote checkout used for the legacy build passes 510 Python tests in505.210 seconds (19 explicit native/Gradle opt-ins skipped) and all308 CI tests in130.482 seconds. Inherited Git controls and native-test opt-ins are removed; both suites finish with the checkout clean. This checkpoint includes the profile capture and earlier Pages wiring; subsequent rotation-input and list-base units retain their separate focused evidence.
- Receipts remain in build/diagnostics/integrated-3c92a67/result.json, with Python/CI log SHA256 4944c0a1facfa9ad5c6d74dee059a23c38a9fb919b868738606d67065cddbccd / 2454764ea1eb3be5f42d57bec76f5488d2da97299989a09e71114f680ff73bcd. No Gradle, remote action or gameplay executes in these regression suites; local success grants no publication or lane qualification.

## Task 10y — preserve asynchronous profile preparation with modern Authlib

- SkinProfilePreparation prepares modern profiles on the supplied background executor and queues registration on the main executor. It retains target identity, the captured local property-map alias/cache, insecure fetch policy, null-result handling and the legacy initial/remote exception boundaries. Modern fetch results copy properties into the original target, including repeated equivalent profiles; unexpected failures still propagate. The legacy class has no operations or Authlib references. Minecraft wiring and model callers remain a separate unit.
- Root repeats strict/offline preprocessing, javac17/21 and the actual Authlib6 default texture decoder with controlled service replies in4.782 seconds. All20 inputs and generated/class observations match; texture types, local cache, executor order, identity, nulls and precise exception behavior pass, and the modern helper rejects Authlib4. Evidence is build/diagnostics/skin-profile-preparation/{closure,root-review}.json and root-probe/. No HTTP, Minecraft client or skin rendering executes. Rollback is the helper and opt-in test.

## Task 12am — share exact-source freshness observation without artifact selection

- newest_exact_source exposes the existing newest successful exact-head run/attempt/controller metadata independently of raw/cache selection. select delegates to that helper and retains its artifact logic and public result. A metadata observation grants no artifact authority; future rotation must repeat it while holding its input leases.
- Root repeats all11 metadata/selection tests and proves with AST comparison that the validation prefix, artifact suffix and every other function remain unchanged. Newer failed/pending/cancelled runs, retries, source drift, wrong controllers and duplicate identities reject without fallback. No rotation route, CLI, workflow or remote action changes. Rollback is the extraction and focused tests.

## Task 10x2 — connect both list widgets to the independent bounds base

- CollectionListWidget and FigureListWidget now extend BoundedSelectionList and carry `>=1.21` branches for `renderWidget` and four-argument `mouseScrolled`, keeping their existing inverse-scale bodies, coordinate maths and offsets. Removing the guards reproduces the recorded baseline sources token-for-token on both widgets, so the legacy lane keeps its previous behaviour.
- The opt-in probe compiles the complete widgets, the bounds base and the current GuiScaleManager against real Minecraft APIs on Java 17 and 21, then executes actual widget callbacks with no window or GL. Both versions print identical native output: `collections: row93, edges13/114/7/108, vertical wheel15, render delegate` and `figures: default xOffset14, vertical wheel20, configuration, render delegate, reset`. Class majors are 61 and 65, and all 198 inputs plus the verification metadata are unchanged after the run.
- Command is `python3 -m unittest tests.test_list_widget_wiring`, green in 10.583 seconds under strict/offline Stonecutter preprocessing with a private Gradle home. Evidence is build/diagnostics/list-widget-wiring/probe01/result.json, SHA256 d31ac4dd0de3c350b84a6591ddce66eaab0e54576f5a7f4e34067d1ff2a25006. Baseline domain methods are not executed and no client, world or gameplay runs; vanilla scrollbar-edge hover differences remain for interaction qualification. Rollback is the two widget declarations and the opt-in probe.

## Task 10y2 — bind the model callers to the modern skin registration adapter

- ClientSkinRegistration exposes a single `>=1.21` entry point over the existing SkinProfilePreparation helper and the captured LocalProfileProperties map; the legacy class keeps no operations and no Minecraft references. FigureModel and FigureBlockModel route their four registration call sites through it while retaining the native `registerSkins` branch for 1.20.1.
- The rewritten probe now compiles both complete models, the detector and every dependency on Java 17 and 21 instead of asserting a compilation boundary: modern models compile with zero pending registration errors, where the inherited checkpoint recorded four. Bytecode assertions confirm two `SkinManager.registerSkins` call sites per model on 17 and two `ClientSkinRegistration.register` call sites per model on 21, the adapter's eight ordered bindings, the `BlockableEventLoop.execute` and `SkinManager.getOrLoad` handles, and the absence of `getGameProfile`/`fetchProfile`. The legacy adapter class contains no `net/minecraft` bytes.
- Command is `python3 -m unittest tests.test_player_skin_reads`, green in 9.506 seconds; 103 inputs and the verification metadata are unchanged. Evidence is build/diagnostics/client-skin-registration/probe/result.json, SHA256 b30f33d9cb8368a2969a476109b797741139d25cf9d4da5253e1475ed012458a. The adapter body is never executed and no rendering, HTTP or skin loading runs, so modern runtime behaviour remains unqualified. Rollback is the adapter, the four call sites and the probe changes.

## Task 10z — preserve the collection screen quad and wheel signature on modern clients

- CollectionSelectionScreen gains a `>=1.21` star-quad path built on `Tesselator.begin`, `addVertex`, `setUv` and `BufferUploader.drawWithShader`, plus the four-argument `mouseScrolled` that forwards the existing inverse-scale mouse transforms. The legacy Tesselator body and the three-argument signature stay active for 1.20.1.
- A new opt-in probe, promoted from the prepared draft into tests/, compiles the complete screen with its current widgets and helpers on Java 17 and 21. The legacy active source stays token-identical to the recorded baseline; the modern quad is token-identical to both historical references; and reverting the quad plus the wheel signature restores the baseline source token-for-token. Bytecode shows four vertex and four UV calls on each version with the era-appropriate upload call, and compiling the legacy source against the modern API fails with exactly nine errors naming `getBuilder()` and `end()`.
- Command is `python3 -m unittest tests.test_collection_screen_api`, green in 9.940 seconds; 197 inputs and the verification metadata are unchanged. Evidence is build/diagnostics/collection-screen-api/probe/result.json, SHA256 088a33690c2e55e545230c37a9458e0710f0952f4229ce233daecda8d5e64ed0. No GL, HTTP, world or gameplay executes and baseline domain methods are not run, so the rendered result is not visually qualified. Rollback is the screen branches and the new probe.

## Task 12an — prove the rotation action lease re-verifies identity before each deletion

- The action lease was implemented in the previous checkpoint but had never been executed. Running it for the first time exposed a real defect: `_RotationReads.stable` returned `sorted(canonical_json(row) ...)`, and `canonical_json` returns bytes, so the caller's `canonical_json(stable(...))` raised `TypeError: Object of type bytes is not JSON serializable` for every non-artifact read. Every `workflow`, `run`, `run_attempt` and `runs` observation, and therefore `recheck` and `pin_current_attempts`, would have failed closed with a type error rather than a policy decision. The fix decodes those canonical rows to text; ordering is unchanged because UTF-8 preserves code-point order.
- New focused tests cover both new surfaces without touching the CLI, the rotation policy or the DELETE route. `_RotationReads` pins repeated policy reads and rejects later drift, malformed artifact responses, duplicate artifact IDs, mutated artifact metadata, and inventory changes that this invocation did not cause, while accepting exactly the IDs it has already deleted; `pin_current_attempts` rejects a historical owner that is no longer the current attempt. The action lease pins the plan to exact IDs, deletes them in ascending order, never touches the retained cache, rechecks the input lease before each deletion, rejects an exact-ID permit that changed only in the per-artifact read, latches failure so a failed lease refuses further deletions, refuses use after the context closes, refuses exhaustion, and rejects a cache whose handoff does not describe the newest exact source.
- Command is `python3 -m unittest tests.test_pages_rotation_actions`, 9 tests green in 0.007 seconds; `python3 -m unittest discover -s tests -p 'test_pages_*.py' -t .` passes 156 tests in 181.121 seconds. The fake API records deletions in memory and performs no network, Git or ZIP work, so this is contract evidence only.
- Limitations: the input lease is substituted in the action tests, so the end-to-end join with real cache descriptors, byte budgets and companion seals is not covered here and remains with the existing rotation-input evidence. No caller wires `current_rotation_actions` yet, the CLI and workflow are unchanged, and no real artifact is deleted. Completed-owner rotation remains a separate contract. Rollback is the decode fix and the new test module.

## Task 10aa — close the 1.21.1 source frontier for the common and Fabric nodes

- A fresh strict frontier run at `612cc18` reduced the inherited `c8ae2b7` result of 136 errors across 21 files to 9 errors in one file, confirming that the committed widget, skin, renderer and screen seams landed. The remainder was `FigurePositionScreen`: `Screen.renderBackground` gained the `(GuiGraphics,int,int,float)` signature, `SkinManager.registerSkins` was replaced by the committed `ClientSkinRegistration` adapter, and `getInsecureSkinLocation` became `getInsecureSkin(profile).texture()`. All three now carry `>=1.21` branches whose legacy bodies are untouched, and `:common:1.21.1:compileJava` reports zero errors.
- The Fabric node then exposed four more incompatibilities. `NbtIo.readCompressed(File)` became `readCompressed(Path, NbtAccounter)`, adopted verbatim from the historical `f425f3c` port. `SavedData.save` gained a `HolderLookup.Provider`, `createFromTag` gained the same parameter, and `DimensionDataStorage.computeIfAbsent` now takes a `SavedData.Factory`. Each is a guarded signature line over the existing shared body, so removing the guards restores the legacy source.
- One deliberate deviation from the historical port: it passed `null` as the factory's `DataFixTypes` with the comment "null for no data fixing". Disassembly of 1.21.1 shows `DimensionDataStorage.readTagFromDisk` calls `DataFixTypes.update` on that value with no null check, and `readSavedData` catches `Exception`, logs, and returns null, after which `computeIfAbsent` builds fresh state. A null type therefore discards every persisted player collection on the first reload after a save file exists. This port passes `DataFixTypes.SAVED_DATA_COMMAND_STORAGE`, vanilla's type for arbitrary stored NBT, which is a no-op whenever the stored data version matches the current one.
- Evidence is build/diagnostics/fabric-1.21.1-probe/attempt15..attempt28. Real strict isolated builds pass for all three configured lanes at the final inputs: fabric-1.21.1 production plus harness in 27.732s, fabric-1.20.1 in 32.808s and forge-1.20.1 in 45.773s, each in a private fixture and APFS-cloned Gradle home with one Gradle process, no daemon, no parallelism and strict dependency verification. The fabric-1.21.1 production jar is a real 5.4 MB remapped artifact carrying the expected `fabric.mod.json` identity.
- Limitations: the fixture lifts the `settings.gradle` guard that still blocks modern game configuration in the real checkout, so this is a selected-node frontier rather than an activated lane. No client, server, world or gameplay runs; the 1.21.1 lane is not qualified, the NeoForge 1.21.1 lane is not configured, and nothing is published. Rollback is the three source ports.

## Task 9a — admit the 1.21.1 runtime natives and the Mojang FreeType origin

- Strict verification blocked `:fabric:1.21.1:remapJar` on fifteen LWJGL 3.3.3 macOS native artifacts that had no recorded checksum. Their SHA-256 values are now recorded from the exact bytes Gradle had already resolved into the local cache.
- Resolution then failed for `lwjgl-freetype-3.3.3-natives-macos-patch.jar`, which Maven Central does not publish: Minecraft 1.21+ links a Mojang-patched FreeType native available only from `libraries.minecraft.net`. The repository policy previously admitted `org.lwjgl` from that host only for NeoForge lanes. It now also admits exactly `org.lwjgl:lwjgl-freetype` for the other loader families, leaving every other LWJGL module on Central and every existing exclusion, host allowlist and HTTPS/scheme rule unchanged. The reviewed-controller digest in `scripts/ci/dependency_policy.py` was updated to the new policy bytes.
- The repository guard requiring Linux coverage for every recorded macOS native then failed. The seven missing `3.3.3-natives-linux` artifacts were fetched from Maven Central over HTTPS, cross-checked against each published `.sha1` sidecar, and recorded by SHA-256, so the lane's CI runtime is covered rather than only this host.
- All 18 dependency-policy tests pass, the broad suites pass 536 Python tests with 23 opt-in skips and 308 CI tests, and all three lanes rebuild green against the final metadata. Limitations: only the three configured lanes are covered, Windows natives for 3.3.3 remain unrecorded because no configured lane requires them, and no publication or release authority follows. Rollback is the policy branch, the digest and the added artifact entries.

## Task 9b — declare the neoforge-1.21.1 lane inputs

- The live matrix now carries a fourth configured lane. `neoforge-1.21.1` declares its own Minecraft/loader identity, Java 21 toolchain, Stonecutter build layout, `:neoforge:1.21.1` production and harness tasks, explicit jar and harness paths, `META-INF/neoforge.mods.toml` metadata bounds and `common`+`neoforge` source routes. A matching runtime row pins loader `21.1.77`, the NeoForge installer, and the architectury/geckolib NeoForge coordinates; `source_routing` gains the `neoforge` canonical and E2E roots; `lane_count` moves to four. `scripts/release/matrix.py` validates the document.
- The NeoForge installer entry records SHA-256 `359b4d48…`, downloaded over HTTPS and confirmed byte-for-byte against the publisher's `.sha256` sidecar.
- Dependency admission was the real cost. Resolving the lane with Gradle's own `--write-verification-metadata sha256` in an isolated fixture added 38 components and 106 artifacts, covering NeoForge, FancyModLoader, accesstransformers, installertools, the NeoForge LWJGL set, ASM, cpw.mods and the architectury/geckolib NeoForge modules. An element-by-element comparison of the previous and regenerated documents confirms zero lost artifacts and zero changed checksums, so the change is purely additive; the visible removals in the textual diff are Gradle re-sorting entries that were previously inserted out of order. All 18 dependency-policy tests pass, including the rule requiring Linux coverage for every recorded macOS native.
- The module still has only its resources: `META-INF/neoforge.mods.toml` and `pack.mcmeta` are written against the lane's declared bounds, and the platform, data, collection, entrypoint, event and item sources remain to be ported. The lane therefore does not build yet and is not qualified; no other lane's behaviour changes. Rollback is the matrix rows, the routing entry, the regenerated metadata and the two resource files.

## Task 10ab — port the neoforge-1.21.1 module and close the four-lane build

- The NeoForge module previously had no main source set. It now carries the full platform contract plus the loader wiring, delivered as two commits to stay inside the review budget. PlatformHelperImpl uses the NeoForge loading API and `FMLEnvironment.dist`; PlayerDataManagerImpl returns `player.getPersistentData()`; PlayerCollectionHelperImpl is ported from the already-1.21-compatible Fabric helper with the offline read moved from Fabric's cardinal-components path to the player `.dat` file's `NeoForgeData` compound under `PlayerDataManager.DATA_KEY`. The `NeoForgeData` tag name was confirmed against the NeoForge 21.1.77 universal jar.
- Three loader API differences were resolved against the real classpath rather than assumed. NeoForge 21.1 removed `Item#initializeClient`, so the custom item renderers Forge attaches per item are bound once through `RegisterClientExtensionsEvent`, choosing the claw, figure or box renderer from the item's block exactly as the Forge subclasses did. Architectury 13.0.8 for NeoForge exposes only `EventBusesHooks` and registers the mod event bus itself, so the Forge `EventBuses.registerModEventBus` call is dropped and the constructor accepts the bus only to bind the entrypoint. `PlayerCollectionGenerator` is a Forge-module class, so the entrypoint calls the common `PlayerCollectionHelper` facade instead. Event subscribers use `net.neoforged.fml.common.EventBusSubscriber` with the MOD and GAME buses.
- All four in-scope lanes now build real production and harness jars in isolated fixtures, one Gradle process each, no daemon, no parallelism, strict dependency verification: fabric-1.20.1 in 31.84s, forge-1.20.1 in 45.03s, fabric-1.21.1 in 26.22s and neoforge-1.21.1 in 42.57s, all with zero compiler errors. The NeoForge production jar is a real 5.4 MB remapped artifact carrying the expected `neoforge.mods.toml` identity, the entrypoint and the platform implementation classes, beside a 36 KB packaged-E2E harness. The broad suites pass 536 Python tests with 23 opt-in skips and 308 CI tests.
- Limitations are unchanged and important. The two modern lanes are still built with the `settings.gradle` guard lifted inside the fixture, so they are selected-node frontiers rather than activated lanes; the real checkout still refuses modern game configuration. No client, server, world or gameplay runs, so no lane is qualified, and the NeoForge runtime behaviour — event bus timing, client extension binding and offline colour reads — has not been observed in a running game. Rollback is the two module slices.

## Task 16a — activate the selected Stonecutter graph for the configured lanes

- `settings.gradle` refused every Stonecutter lane at `projectsLoaded` with "Selected Stonecutter graph requires common, loader and E2E node adapters before game configuration". All three adapters the guard names now exist and build for both modern lanes, so the guard is removed and the modern graph configures in the real checkout. The reviewed-controller digest for `settings.gradle` in `scripts/ci/dependency_policy.py` is updated to the new bytes.
- The guard is not load-bearing for unconfigured lanes. `blockpopsLane` must name a row in the matrix `artifacts` list, and the eight out-of-scope targets have no such row, so `scripts/release/matrix.py` still rejects them before Gradle configures anything. The remaining selection, schema, routing and loader-inventory checks in `settings.gradle` are untouched.
- All four configured lanes now build real production and harness jars against the unpatched controller, with no fixture edit other than injecting the working-tree `settings.gradle` itself: fabric-1.20.1 in 32.98s, forge-1.20.1 in 46.44s, fabric-1.21.1 in 28.76s and neoforge-1.21.1 in 45.35s, each with zero compiler errors, one Gradle process, no daemon, no parallelism and strict dependency verification. This is the step that turns the two modern lanes from selected-node frontiers into configured lanes. 536 Python tests pass with 23 opt-in skips and 308 CI tests pass.
- The migration stays in `preparing` mode. Switching to `shared` is a separate decision that the change's own rules bind to qualification of every declared target, and eight targets remain unconfigured by explicit scope. No client, server, world or gameplay runs here either, so activation of the build graph is not lane qualification and grants no release or publication authority. Rollback is the guard block and the digest.

## Task 12a — give consumers a configured-lane projection without changing any dispatch

- The reader offered only two collective scopes: `legacy`, which selects the two declared legacy nodes, and `full`, which calls `require_complete()` and refuses while any declared target is unconfigured. With eight targets deliberately out of scope, no consumer could ask for "every lane that is actually configured", so the two modern lanes were invisible to CI and E2E projections even though they build.
...
## Task 10ac — port the 1.21.2+ block and interaction seams

- The historical `feature/multi-version-stonecraft` branch (tip `50eb7fd`, 42 commits ahead of `master`, never merged) carries per-version snapshots for all six Minecraft versions. Its snapshots are taken at one commit, so diffing `common/src/v1_21_1` against `common/src/v1_21_4` isolates the version-adaptation delta from feature work. That branch is a behaviour and coordinate reference only: design.md forbids `src/v*` snapshots, so the port translates its findings into preprocessor conditionals and narrow adapters in the shared tree.
- Three API changes explain every 1.21.4 error in `common`. `ItemInteractionResult` was removed when 1.21.2 restored `InteractionResult` on `useItemOn`; `DirectionProperty` was replaced by `EnumProperty<Direction>`; and `BlockEntityWithoutLevelRenderer` was replaced by `SpecialModelRenderer`. The first two are syntax-level and are handled here.
- The existing `BlockInteractionResults` adapter gains a third era. Stonecutter `elif` chains were verified to work before relying on them: `>=1.21.2` returns the result unchanged and rejects the two constants outside the legacy contract, `>=1.21` keeps the `ItemInteractionResult` mapping, and 1.20.1 keeps an empty class. The three blocks gain matching arms for their `FACING` property, their `DirectionProperty`/`EnumProperty` import and their `useItemOn` signature, with the shared body untouched.
- `:common:1.21.4:compileJava` drops from 40 errors across seven files to 12 across three, all of them the `SpecialModelRenderer` rewrite, which is a structural change and is deliberately left to its own adapter slice. fabric-1.21.1 and fabric-1.20.1 both still build green at 27.12s and 34.02s, so the new era arm does not disturb either existing lane.
- Limitations: no 1.21.4 lane builds yet, the three item renderers are unported, the loader modules have not been examined for 1.21.4, and nothing here is qualified or run in a game.

## Task 10ad — port the 1.21.4 item renderers and expose the real frontier

- 1.21.4 replaced `BlockEntityWithoutLevelRenderer`, a base class with `renderByItem`, by the `SpecialModelRenderer<T>` interface, whose only members are `extractArgument(ItemStack)` and a seven-argument `render`. The historical branch answered this by writing a separate class per version with its own `RenderData` record. Following design.md, the shared tree instead keeps one class whose shell is conditional: it implements `SpecialModelRenderer<ItemStack>` from 1.21.4 and extends the old base class below that, with the existing `renderByItem` body untouched and reused by the new entry point. `extractArgument` returns `stack.copy()` so the render pass observes the same snapshot the legacy path received. `Minecraft.getTimer()` became `getDeltaTracker()` in 1.21.2, so the partial-tick read gained a third arm.
- Fixing the renderers moved the frontier rather than closing it. javac had been stopping at the unresolved renderer classes; with them resolved it now attributes the rest of the module and reports 114 errors across 22 files. That number is the first honest measurement of the 1.21.4 surface, not a regression: the previous count of 12 was masking it.
- One defect of this port's own making was found and fixed. `InteractionResult` is no longer an enum in 1.21.4 but a sealed interface of constants, so the adapter's `switch` over it was invalid; the 1.21.2+ arm is now simply the identity, which is also what the restored `useItemOn` contract means.
- What remains for 1.21.4 is genuine API work rather than syntax: GeckoLib 4.8.2 changed `GeoModel.getTextureResource` and several overridden model methods, `BlockBehaviour.getCloneItemStack` changed shape, both block entities no longer override their previous methods, and the list widgets, `SkinModelDetector` and several screens have their own differences. fabric-1.21.1 and fabric-1.20.1 both still build green, so none of this has disturbed the two established lanes.

## Task 10ae — close the 1.21.4 common frontier

- `:common:1.21.4:compileJava` now reports zero errors. Reaching it took fourteen distinct API migrations, each confirmed against the real 1.21.4 jar or the historical branch rather than guessed, and each expressed as a `>=1.21.2` arm that leaves the 1.20.1 and 1.21.1 bodies untouched.
- Blocks and interaction: `RenderShape.ENTITYBLOCK_ANIMATED` and the `getRenderShape` override are gone for GeckoLib blocks; `InteractionResult.sidedSuccess` disappeared with the enum and becomes plain `SUCCESS`; `Level.getMaxBuildHeight` became `getMaxY`; `getCloneItemStack` gained an `includeData` flag; `BlockEntity.saveToItem` no longer exists, so the two block entities keep the method as a local helper without `@Override`.
- Registry and GeckoLib: `BlockEntityType.Builder` was removed, so a new `>=1.21.2`-only `PlatformHelper.createBlockEntityType` with a `BlockEntityFactory` interface routes the three registrations through `FabricBlockEntityTypeBuilder` on Fabric and the direct `BlockEntityType` constructor on NeoForge. GeckoLib 4.8.2 added a renderer argument to `getModelResource` and `getTextureResource`; call sites forward the renderer where they have one and pass null from `getRenderType`, which our models ignore.
- Widgets, screens and textures: `getScrollAmount` became `scrollAmount`, `getMaxPosition` became `contentHeight`, `getScrollbarPosition` became `scrollBarX`, and `getRowTop` became public. `BoundedSelectionList` was restructured so its scroll bound sits outside the shared modern block and overrides `maxScrollAmount` on 1.21.4. `GuiGraphics.blit` and `blitSprite` take a `RenderType` function first and read UV before the on-screen size, so both call sites are written out per era rather than sharing continuation lines. `RenderSystem.setShader` is gone, `RenderSystem.clear` lost its boolean, `TextureManager.register` returns void, `HttpTexture` was replaced by `SkinTextureDownloader` so that detection branch is legacy-only, and `NativeImage.getPixelRGBA` became `getPixel`.
- Two defects of this port's own making were caught by the compiler and fixed: a nested block comment inside a Stonecutter arm silently closed the outer comment, and an arm was added inside an already-commented region where nesting is not possible. Both now use line comments or a restructured block.
- fabric-1.21.1 and forge-1.20.1 still build green at 39.29s and 66.17s. The fabric-1.21.4 lane itself does not build yet: Fabric API removed `BuiltinItemRendererRegistry`, and 1.21.4 registration needs each renderer to expose an `Unbaked` map codec through `SpecialModelRenderers.ID_MAPPER` plus item model JSON files. That is the next slice.

## Task 10af — complete the two 1.21.4 lanes

- Both 1.21.4 lanes now build production jars. The remaining work was item-renderer registration, which 1.21.4 moved from loader-specific hooks to a vanilla id-mapped codec. Each of the three renderers gains a `>=1.21.4` arm exposing an `ID` and an `Unbaked` record whose `MAP_CODEC` bakes the renderer, matching the shape the historical branch shipped.
- Fabric API removed `BuiltinItemRendererRegistry` in 1.21.4, so `registerItemRenderers` is written per era: the modern arm puts the three codecs into `SpecialModelRenderers.ID_MAPPER`, while the legacy arm keeps its per-item registration untouched. NeoForge replaced `RegisterClientExtensionsEvent` for items with `RegisterSpecialModelRendererEvent`, so the NeoForge client gains a modern arm registering the same three ids and keeps the client-extension path below 1.21.4.
- All six configured lanes build with zero compiler errors: fabric-1.20.1, forge-1.20.1, fabric-1.21.1, neoforge-1.21.1, fabric-1.21.4 and neoforge-1.21.4.
- Limitations worth stating plainly. 1.21.4 binds special renderers to items through item model JSON, and those model files have not been written, so the custom item rendering is registered but not yet referenced by any item; the 1.21.4 lanes build and package but their item appearance is unverified. As everywhere else in this change, no client, server or world has been run, so neither 1.21.4 lane is qualified.

## Task 15a — first real packaged gameplay evidence, from a developer Mac

- The packaged E2E was run for real on this machine, not simulated. Each lane went through the full pipeline in a clean checkout: `build_matrix.py`, `verify_release.py` staging, then `e2e/orchestrator.py --packaged` installing a production Minecraft runtime, starting a dedicated server and a client with the staged production and harness jars, and driving the five-step `ui-regression` scenario.
- Three lanes reach `harness: pass` with all five steps passing, which is the first gameplay-level evidence in this change: forge-1.20.1, neoforge-1.21.1 and neoforge-1.21.4. The steps cover the organic first-join colour prompt with its sixteen colours, selecting purple, the server round trip that synchronizes the purple World Players figure, a real block interaction opening the collection screen, and the settings modal.
- Seven defects were found that no build or compile check could see, each fixed in its own commit: the Forge favourite colour never reaching the collection because the write and read used different stores; blocks and items missing their registry id on 1.21.2+; NeoForge `loaderVersion` carrying the loader version instead of the javafml version, in both the production and harness manifests; a 1.21.1 loader older than its own GeckoLib requires; production manifests hard-coded to the 1.20.1 era so no modern lane could stage; Stonecutter per-node Gradle state marking every modern checkout dirty; and a NeoForge entrypoint name the artifact contract did not accept.
- Three environment limits are not defects and are recorded so nobody re-diagnoses them. Every `*.fabricmc.net` host is unreachable from this network, so no Fabric lane can install its runtime here, although Central, Mojang, NeoForged and Cloudsmith all respond. The contracted 1600x900 client window does not fit this display's 1512 logical points, so the pixel probes compare regions that never match and no visual assertion can pass locally. Finally, `-Dmixin.debug.countInjections=true`, which the runtime sets to exercise our own injector expectations, makes GeckoLib's optional Forge-variant armour mixin fail closed on 1.21.4 and takes the client down before any BlockPops code runs; removing it locally is what let the 1.21.4 scenario complete. That flag is unchanged in the repository, because narrowing it is a policy decision for the owner rather than a defect to patch.
- Scope and honesty: only the `ui-regression` scenario ran, only on this machine, and the visual probes never passed anywhere, so no lane is qualified and no visual review evidence exists. The Fabric lanes have no gameplay evidence at all.

## Task 10ag — configure and complete the two 1.21.5 lanes

- Two checksum gaps that predate this task were found first, because a lane cannot be measured while the lanes beside it are broken. Moving the 1.21.4 lanes to GeckoLib 4.8.5 changed what Fabric resolves — 4.8.5 declares a newer Fabric API than the matrix pins, so conflict resolution selects 0.115.1+1.21.4 and its whole module graph — and none of it had a checksum, so fabric-1.21.4 could not configure at all. A NeoForge sources jar from the 1.21.1 loader bump was missing the same way. Both are admitted from the bytes Gradle itself resolved.
- The lanes are configured from the coordinates the historical `feature/multi-version-stonecraft` branch shipped for this era: Architectury 16.1.4, Fabric API 0.126.0+1.21.5, NeoForge 21.5.82 and GeckoLib 5.1.0, with the Fabric loader left at the 0.17.0 the other modern lanes already verify. The NeoForge installer checksum was taken from the downloaded bytes and agrees with the publisher's own `.sha256` sidecar.
- GeckoLib 5 is a different contract, not a renamed one. `AnimatableManager` and `AnimationController` moved into the animatable package and the controller no longer takes its animatable; `GeoModel.getModelResource` and `getTextureResource` lost the animatable entirely and are handed a `GeoRenderState`. Each model keeps one shared `resolveModel`/`resolveTexture` body, computes it in `addAdditionalStateData` and carries the answer across as render data, so the resolution logic is written once and only the entry points differ per era.
- GeckoLib 5 also moved its assets. It scans `geckolib/models` and `geckolib/animations` and addresses them by bare path; GeckoLib 4 scans `geo` and `animations` and wants the full path with its extension. Rather than ship two copies of every model, `GeoAssets` names assets by bare path, reconstructs the older layout there, and `:common:processResources` re-roots the files for lanes whose GeckoLib expects the newer one. Ids declared in collection data go through the same adapter, so no data file changed.
- 1.21.5 turned every `CompoundTag` and `ListTag` getter into an `Optional` and removed the typed `contains` overload, which reaches almost every persisted field in the mod. One `TagReads` adapter answers all of it: each call site states the value the old getter used to answer for a missing entry, so no guard, default or persisted key changed.
- The rest of 1.21.5 is vanilla rendering. `BufferUploader` and the `RenderSystem` blend, depth and scissor state are gone, so the scrolling star background is one tinted `blit` through `RenderType::guiTextured`, the star texture sets its own filtering and wrapping, the link button and the settings modal stop setting global state, and a logo's size comes from the uploaded texture instead of a raw GL query. `ClickEvent` became a sealed hierarchy carrying a URI, `Item.appendHoverText` now receives a tooltip consumer and a display, `onRemove` became a hook that runs after the block is gone, and `SavedData` is saved through a codec instead of a save/load pair.
- The GeckoLib 5 renderer contract was the last slice. Its render methods only see the render state, so the box renderer holds the box being drawn for the length of one render pass — which is what the historical branch's own 1.21.5 renderer does — and the figure renderer answers "is there a figure yet" once, in `render`, where the block entity is still in hand. The arm-visibility rule and the logo placement became shared bodies both eras call.
- Stonecutter nesting was verified rather than assumed before relying on it: the generated 1.21.5 source escapes inner comment markers as `/^ … ^/` when an outer arm is commented, so an era arm may contain era arms.
- All eight lanes build production jars with zero compiler errors: fabric-1.20.1, forge-1.20.1, fabric-1.21.1, neoforge-1.21.1, fabric-1.21.4, neoforge-1.21.4, fabric-1.21.5 and neoforge-1.21.5. Both 1.21.5 E2E harness jars build as well.
- One environment finding worth recording, because it contradicts an earlier one. `maven.fabricmc.net` is not reachable over public DNS from this network, but it is reachable at the address the diagnostic host file pins, and `maven2.fabricmc.net` serves byte-identical content — checked by comparing a loader jar and an intermediary jar against the checksums already in `gradle/verification-metadata.xml`, and by comparing the 1.21.5 intermediary mappings against the copy the historical build fetched months ago. `FRONTIER_ALLOW_NETWORK=1` is therefore worse than the default here, because it removes the pin and falls back to addresses that do not answer.
- Limitations, stated plainly. No client, server or world has run on 1.21.5, so neither lane is qualified. The 1.21.4 caveat carries over: special item renderers are registered but no item model JSON references them. The settings modal relies on draw order rather than the depth buffer on 1.21.5, and the star background's seamless wrap now depends on the texture's own address mode; only a running game can confirm either.

## Task 10ah — 1.21.6, 1.21.7 and 1.21.8, and where the version ladder stops

- 1.21.6 is the second structural break after 1.21.5. Block entity persistence moved to `ValueOutput`/`ValueInput`, so each block entity keeps one body over a `FieldSink` and a `FieldSource`; item stacks still hand the same fields over as a raw tag, and the chunk-sync tag is built through the value output and merged. `ServerPlayer.serverLevel` became `level`. The GUI pose became two-dimensional, the batch flush disappeared, blits select their shader by render pipeline, `Lighting` became an instance the game renderer owns, and a special model renderer must declare its extents. NeoForge 21.6 picks the event bus from the event type, so `@EventBusSubscriber` no longer names one.
- The three-dimensional figure preview was the one piece with no obvious answer: `GuiGraphics.pose()` no longer yields a `PoseStack`, and the historical branch solved it with a picture-in-picture renderer, a mixin and reflection into vanilla's private texture fields. The sibling Quick-Skin mod, which supports every version through 26.3, solves the same problem by building its own `PoseStack` and drawing straight into the buffer source. That is what this port does, so no mixin, no reflection and no new render subsystem were needed. The colour swatch keeps its placement and scale but loses its extra tilt, because a two-dimensional pose cannot express it.
- One repository policy change: NeoForge's own module metadata imports the LWJGL bill of materials, which Mojang's library host does not publish. That one dependency-management document is now allowed from NeoForge's own repository, and every LWJGL artifact still comes from Mojang.
- 1.21.7 and 1.21.8 needed no source change at all. Every API the mod touches is unchanged from 1.21.6, so only coordinates and checksums are new.
- The declared migration scope grew from twelve targets to eighteen. About thirty controller tests hard-coded the old number; where it is incidental they now count eighteen, and where it was a snapshot of a moving list they bind the contract instead. One of those, the configured-lane test, had been failing since the 1.21.4 lanes were configured, because it named the four lanes that existed when it was written.
- Eight lanes now build production jars with zero compiler errors: fabric-1.20.1, forge-1.20.1, and both loaders for 1.21.1, 1.21.4, 1.21.5, 1.21.6, 1.21.7 and 1.21.8.

### Where the ladder stops

The remaining targets are declared and unresolved. Only one of them is blocked from outside this change's reach; the other two are unported work. In both of those cases the first reading claimed a hard external blocker, and neither claim survived being checked a second time:

- **1.21.9** — GeckoLib never published a build for it. BlockPops is a GeckoLib mod end to end, so there is nothing to compile against.
- **1.21.10 and 1.21.11** — reachable, not yet ported. The first reading of this was wrong: it said the build could not get there and that moving Loom and Gradle was its own change. It was tried instead, and the move cost nothing — Gradle 9.7.1, Loom 1.17.480 and Architectury 3.5.170 build all eight existing lanes with zero compiler errors and no build script change. `:common:1.21.11:compileJava` now configures and reports 820 errors, where before it could not configure at all. Those errors are a genuine API surface and the port is a substantial piece of work in its own right, described below. 1.21.10's only GeckoLib is an alpha besides.
- **26.1 through 26.3** — reachable, not yet ported. The first reading of this was wrong as well: it said GeckoLib publishes 26.x to Modrinth and CurseForge only, and called that the ladder's real ceiling. The 404s it rested on were real, but they answered the wrong question. GeckoLib renamed its group at the 26.1 boundary — up to 1.21.11 it publishes as `software.bernie.geckolib`, from 26.1 as `com.geckolib`, with the Java package renamed to match — and every probe had asked for the old group's path. The same Cloudsmith Maven serves `com/geckolib/geckolib-<loader>-26.1` at 5.5, `-26.2` at 5.5.1 through 5.5.6 and `-26.3` at 5.5.6, and the jars download. Two things made the wrong reading look confirmed: that Maven is not browsable, so a wrong path and a missing artifact return the same 404; and this repository's own content filters pinned that host to `software.bernie.*` while excluding the same pattern from Maven Central, so a correct coordinate would have failed to resolve too and would have read as further proof. Both filters now allow `com.geckolib`. The 26.3 module metadata declares no transitive dependencies, so 26.x brings no repeat of the Fabric API checksum surprise from 1.21.4.

The 1.21.11 frontier, measured on the new toolchain, is dominated by one rename: 540 of the 820 errors are `ResourceLocation`, which 1.21.9 renamed to `Identifier`. The rest are `RenderType` moving into its own package, `GameProfile` losing its getters to an authlib bump, `SkinManager.getInsecureSkin` becoming `createLookup`, `ServerPlayer.getServer` and `hasPermission` disappearing, `GuiGraphics.renderOutline` going away, the selection lists losing `itemHeight` and `headerHeight`, and GeckoLib 5.4 changing both its renderer base and `GeoModel.addAdditionalStateData`. Stonecutter 0.7.11 exposes a replacement container that looked like the right tool for the rename; it accepts the rule and never rewrites the sources, so it was removed rather than left looking like it works.

The 1.21.10 measurement is worth keeping even though the lane is not configured: `:common:1.21.10:compileJava` reports 284 errors across 29 files, and they are a real API surface rather than syntax — `GameProfile` lost its getters to an authlib bump, `PlayerSkin` moved out of the client package, `SkinManager.getInsecureSkin` became `createLookup`, `GuiGraphics.renderOutline` is gone, the selection lists lost `itemHeight` and `headerHeight`, and GeckoLib 5.3 changed the renderer base again. That is the size of the next port once the build can reach it.

## Task 15b — packaged gameplay evidence for 1.21.4, 1.21.5 and 1.21.8, on this Mac

- The packaged E2E was run for real again, through the whole pipeline in a clean checkout: `build_matrix.py`, `verify_release.py` staging, then `e2e/orchestrator.py --packaged` installing a production Minecraft runtime, starting a dedicated server and a client and driving the `ui-regression` scenario.
- Two defects were found before any scenario could run at all.
  - The isolated frontier builds from a Gradle home whose Loom cache is already primed, so Loom reads the intermediary mappings from disk and never resolves `net.fabricmc:intermediary` as a module. The release pipeline builds each lane in a fresh Gradle home, where it does, and that artifact had no checksum for the 1.21.5 through 1.21.8 eras. Every one of those lanes failed dependency verification before compiling. This is worth remembering as a method limitation: a green frontier is not a green release build.
  - `-Dmixin.debug.countInjections=true` is a global switch. It was set to exercise this mod's own injector expectations, but it validates every other mod's mixins too, and GeckoLib's Forge-variant armour layer is optional by design. From 1.21.4 onward it reports "expected 1 invocation(s) but 0 succeeded" and takes the client down during resource reload, before any BlockPops code runs. It is removed: `blockpops.mixins.json` already declares `defaultRequire: 1`, so this mod's own injectors still fail closed.
- With those fixed, three lanes start, join, and reach the favourite-colour screen. The comparison is what matters:

| Lane | mean luma | bright fraction | Screen |
|---|---|---|---|
| neoforge-1.21.4 | 34.00 | 0.000 | correct: title, description, sixteen 3D box models |
| neoforge-1.21.5 | 36.43 | 0.000 | correct: title, description, sixteen 3D box models |
| neoforge-1.21.8 | 38.43 | 0.080 | wrong: no title or description, swatches are flat squares, star pattern over the panel |

- The 1.21.4 and 1.21.5 numbers sit just over the contracted `mean_luma <= 32`, with no bright pixels at all. That is the display limitation already recorded: the contracted 1600x900 client window does not fit this display's 1512 logical points, so every capture is scaled and every region probe is measured against the wrong pixels. Those two are visually correct in the screenshots and fail only the threshold.
- 1.21.8 is a genuine regression from this change's own 1.21.6 port, and the 1.21.5 result is what proves it: the same screen, the same scenario, the same GeckoLib 5 model code, correct on 1.21.5 and wrong on 1.21.8. 1.21.6 draws the GUI from a deferred render state ordered by stratum rather than by call order, so a full-screen background submitted first can still land on top of the panel. Closing the background's layer with `nextStratum` moved it from 45.98 to 38.43 and brought the chat and social overlays back, but the panel's title, its description and its sixteen item models are still not reaching the top layer.
- A third defect was then found and fixed from the same screenshots. Until 1.21.6 a colour whose alpha byte was zero was treated as opaque; 1.21.6 dropped that fixup, so every `0xFFFFFF` the screens draw text with became fully transparent. With the alpha stated, the favourite-colour screen's title and both description lines render again on 1.21.8, and `mean_luma` reads 42.23.
...
## Task 15h — the declared scope matches what can be built

- 1.21.10 and 1.21.11 are no longer declared targets, for either loader. Neither can be built: 1.21.10 only ever received GeckoLib alphas, and GeckoLib 5.4.5 for 1.21.11 bounds its block renderer by an intersection type that no render state satisfies. Keeping them declared left four permanently unresolved targets, so a full plan could never be produced and the migration could never be complete. They can be declared again the day GeckoLib ships a usable build for them.
- With twenty targets and twenty configured lanes, `configuration_complete` is true and `build_matrix.py --plan` now returns a full, non-partial twenty-lane plan where it used to stop on unresolved targets.
- CI still dispatches `legacy` - the two 1.20.1 lanes - because the matrix remains in `preparing`, which is what selects that default. Switching to `shared` is task 16, and it is more than a flag: it also moves the two 1.20.1 lanes from the legacy build layout to Stonecutter, and replaces the legacy projections in the Gradle scripts and the CI consumers. Removing these targets was that task's first precondition, not the task itself.

## Task 8 follow-up — the trusted PR gate reads the preparing matrix

- **What broke.** Task 8 enrolled the live matrix as schema 2 `preparing` (`466426b`), but two readers on the trusted PR gate's own path were never adapted: `gate_controller.validate_controller_parity` and `pr_gate._matrix_for_identity` still called `load_matrix_bytes`, whose `validate_matrix` keeps schema 2 inventory-only on purpose. Every evaluation therefore stopped at `handle-pr-gate-result.yml` → `pr_gate.py evaluate` → `validate_pr_tree` / `validate_controller_upgrade_tree` → `validate_controller_parity` → `load_matrix_bytes` → `validate_matrix` → "schema 2 is inventory only; execution consumers are not supported yet", and both `Trusted PR / ...` contexts were published as failures within seconds, for ordinary and controller-upgrade PRs alike. The Build and Packaged E2E identity jobs and `e2e_job_graph.expected_jobs` already read `load_matrix_document`, so they were never affected.
- **What changed.** `MatrixDocument.trusted_gate_projection()` and `load_trusted_gate_matrix_bytes()` give the gate an explicit view instead of weakening the guard: schema 1 keeps every row; schema 2 is accepted only in `preparing` mode on the integration branch and projects exactly the branch policy and the legacy nodes `fabric-1.20.1` and `forge-1.20.1`. `shared` (until task 16 extends the projection), any other mode, a release-role schema 2 and every malformed document raise `MatrixError`. Parity and PR identity read the projection; `evaluate_restricted_transition` now calls `load_matrix_bytes` itself, so restricted evidence graphs stay schema-1 only; release-sync topology and `sync_merge.py` are untouched and still refuse a schema-2 master. `validate_matrix`, `execution_supported` and every other consumer are unchanged.
- **Equivalence.** On the preparing master the gate derives what it derived from the last schema-1 matrix (`466426b^`): 24 fixed protected paths plus `fabric/build.gradle`, `fabric/src/e2e`, `forge/build.gradle`, `forge/src/e2e`; controller-upgrade loader roots `fabric` and `forge`; the four Build jobs and the Packaged E2E graph with `fabric-1_20_1--pr-behavior` and `forge-1_20_1--pr-behavior`. Only parity's loader paths, the PR identity/branch policy and the controller-upgrade loader roots read the projection; the job graphs come from the full document's default `legacy` scope, which selects the same two lanes. One difference is deliberate and predates this slice: the loader-bootstrap contract reads every configured lane of the inventory (task 8), so an ordinary edit to `neoforge/build.gradle` or `neoforge/src/e2e` must match its contract digest, which the schema-1 gate never required. The simulation below shows that edit admitted on the schema-1-equivalent master and refused on the preparing one. The gate is stricter there, never looser, and this slice does not relax it.
- **Files.** `scripts/release/matrix.py`, `scripts/ci/gate_controller.py`, `scripts/ci/pr_gate.py`, `scripts/ci/tests/test_gate_controller.py` (its parity mocks follow the renamed reader), the new `scripts/ci/tests/test_pr_gate_schema2.py`, and `docs/operations.md`. All six are controller-upgrade paths; this openspec record is not one, so it cannot travel in the controller-upgrade PR.
- **Evidence** (Python 3.13 venv with the hash-locked dependencies, LF checkout, `PYTHONDONTWRITEBYTECODE=1`):

| Check | Command | Result |
|---|---|---|
| Baseline at `9ba22a2` | `python scripts/ci/parallel_unittest.py -t . scripts/ci/tests tests` | exit 0, 907 tests OK, 24 skipped |
| RED | the new test module against `9ba22a2` plus only the new matrix API | exit 1: every preparing-master case fails (4 errors, 4 subtest failures) on "schema 2 is inventory only"; the schema-1, projection and `shared` cases pass |
| GREEN | `python -m unittest scripts.ci.tests.test_pr_gate_schema2` | exit 0, 11 tests OK |
| Mutations | projection returns every configured lane; accepts `shared`; accepts a release-role schema 2; restricted transition drops its schema-1 read; parity keeps `load_matrix_bytes` | 5 of 5 killed |
| Full suite | `python scripts/ci/parallel_unittest.py -t . scripts/ci/tests tests` | exit 0, 918 tests OK, 24 skipped |

- **Offline simulation** (throwaway commits in a scratch repository that borrows the object store through alternates; real policy code on real Git objects, only GitHub's API replaced; job records are the graph each evaluator derives from `466426b^`). `9ba22a2`'s evaluator refuses an empty ordinary PR, this fix and PR #10 (`c667c63`) with the chain above. The fixed evaluator admits all three with both contexts `success`, admits an empty ordinary PR and PR #10 merged onto the landed fix, reproduces the schema-1 reference exactly, and refuses this fix plus this openspec record as a controller upgrade (`openspec/...` is a non-controller path). An ordinary edit to `neoforge/build.gradle` is admitted on the schema-1-equivalent master and refused on the fixed preparing master by the loader-bootstrap contract.
- **Delivery.** This is an out-of-procedure repair. `docs/operations.md` sends every repair through the controller-upgrade process, but the deployed evaluator refuses every PR while `master` is schema 2, this one included, so that process cannot admit the evaluator that implements it. It landed on 2026-09-26 as the owner-authorized merge of PR #12 (merge commit `53ed97d` over the reviewed commit `47d9dab`, six paths); that PR's own `Trusted PR` contexts were red because they ran the evaluator this commit fixes. The same commit adds the exception to the bootstrap-limitation section of `docs/operations.md`: same governance process and limits as the first installation, then a fresh-PR exercise of both protected gates. This record follows as an ordinary PR once the fix is live.
- **Remaining.** Task 16 must extend the projection for `shared`. Release synchronization (topology/`sync_merge.py`) still refuses a schema-2 master; that belongs to tasks 16/17 and was not enabled here.
- **Rollback.** Revert the six paths together; the gate then returns to refusing every PR while the matrix is schema 2.
