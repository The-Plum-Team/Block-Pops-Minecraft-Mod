...
### Why the ladder stops at 1.21.8

The remaining targets are declared and unresolved for reasons outside this change's reach, each checked rather than assumed:
...
- **26.1 through 26.3** — GeckoLib publishes these to Modrinth and CurseForge only, and this is the ladder's real ceiling. Its own Maven carries `geckolib-<loader>-<mc>` up to 1.21.11 at 5.4.5 and returns 404 for every 26.x coordinate; Maven Central has nothing under `software.bernie.geckolib`; blamejared, saps.dev, MinecraftForge's Maven and GitHub Packages have nothing either; and Modrinth's Maven keys artifacts by project version alone, which cannot distinguish the Fabric 26.2 build from the NeoForge 26.3 build that share version 5.5.6. BlockPops is a GeckoLib mod end to end, so no amount of build work reaches 26.x until GeckoLib publishes it somewhere a build can resolve.
...
- What is left on 1.21.6 and later is bounded and named: the star background still draws over the panel rather than behind its translucent fill, and the sixteen colour swatches render as flat squares because `graphics.renderItem` under a two-dimensional pose does not draw the block model. Both are in the deferred-GUI seam this change introduced for 1.21.6, both are visible in the screenshots beside the correct 1.21.5 capture, and neither affects any version below 1.21.6.
