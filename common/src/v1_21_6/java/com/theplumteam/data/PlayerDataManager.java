...
public class PlayerDataManager {
    ...
    public static IPlayerDiscovery getDiscovery(Player player) {
        ...
        if (persistentData.contains(DATA_KEY)) {
            CompoundTag dataTag = persistentData.getCompoundOrEmpty(DATA_KEY);
            discovery.deserializeNBT(dataTag);
        }
    ...
    public static void copyData(Player from, Player to) {
        ...
        if (fromData.contains(DATA_KEY)) {
            CompoundTag dataCopy = fromData.getCompoundOrEmpty(DATA_KEY).copy();
            getPersistentData(to).put(DATA_KEY, dataCopy);
