...
BlockPops to the game classpath.
"""
...
from e2e.dependency_integrity import DependencyIntegrityError, verified_sha256
from e2e.runtime_store import (
...
_PRIMARY_OPAQUE_STARS_PROBE = next(iter(OPAQUE_STARS_PROBES.values()), None)
OPAQUE_STARS_BACKGROUND_REGION = (
    _PRIMARY_OPAQUE_STARS_PROBE.region if _PRIMARY_OPAQUE_STARS_PROBE else None
)
OPAQUE_STARS_MAXIMUM_MEAN_LUMA = (
    _PRIMARY_OPAQUE_STARS_PROBE.maximum_mean_luma
    if _PRIMARY_OPAQUE_STARS_PROBE else None
)
OPAQUE_STARS_BRIGHT_LUMA = (
    _PRIMARY_OPAQUE_STARS_PROBE.bright_luma if _PRIMARY_OPAQUE_STARS_PROBE else None
)
...
RUNTIME_STORE_ENV = "BLOCKPOPS_E2E_RUNTIME_STORE"
RUNTIME_STORE_MAX_AGE_ENV = "BLOCKPOPS_E2E_RUNTIME_STORE_MAX_AGE_SECONDS"
RUNTIME_STORE_MAX_BYTES_ENV = "BLOCKPOPS_E2E_RUNTIME_STORE_MAX_BYTES"
...
def runtime_store_cache_root(env: Mapping[str, str] | None = None) -> Path:
    ...
    if sys.platform == "darwin":
        return (Path.home() / "Library" / "Caches" / "BlockPops" / "e2e").resolve()
    if os.name == "nt":
        local_app_data = selected_env.get("LOCALAPPDATA")
        base = Path(local_app_data) if local_app_data else Path.home() / "AppData" / "Local"
        return (base / "BlockPops" / "e2e").expanduser().resolve()
    xdg_cache = selected_env.get("XDG_CACHE_HOME")
    base = Path(xdg_cache).expanduser() if xdg_cache else Path.home() / ".cache"
    return (base / "blockpops" / "e2e").resolve()
...
def java_executable(major: int) -> str:
    candidates = [
        os.environ.get(f"BLOCKPOPS_JAVA_{major}"),
    ...
    if found and detected_java_major(found) == major:
        return found
    raise RuntimeFailure(
        f"Java {major} not found; configure BLOCKPOPS_JAVA_{major} or JAVA_HOME_{major}_X64"
    )
...
def maven_dependency_url(dependency: Mapping[str, Any]) -> tuple[str, str, str, str, str]:
    """Resolve one matrix-owned Maven coordinate without introducing loader assumptions."""

    coordinate = dependency.get("coordinate")
    repository = dependency.get("repository")
    if not isinstance(coordinate, str) or coordinate.count(":") != 2:
        raise RuntimeFailure(f"invalid runtime dependency coordinate {coordinate!r}")
    if (
        not isinstance(repository, str)
        or not repository.startswith("https://")
        or not repository.endswith("/")
    ):
        raise RuntimeFailure(f"invalid runtime dependency repository {repository!r}")
    ...
    url = (
        repository
        + group.replace(".", "/")
        + f"/{name}/{encoded_version}/{filename}"
    )
    return group, name, version, url, filename
...
def runtime_dependencies(
    ...
    raw_dependencies = row.get("runtime_dependencies")
    if not isinstance(raw_dependencies, list) or not raw_dependencies:
        raise RuntimeFailure("runtime row has no locked runtime dependencies")
    specifications = [maven_dependency_url(dependency) for dependency in raw_dependencies]
...
def installed_version_id(row: dict[str, Any]) -> str:
    if row["loader"] == "fabric":
        return f"fabric-loader-{row['loader_version']}-{row['minecraft']}"
    if row["loader"] == "forge":
        forge_loader = row["loader_version"].removeprefix(f"{row['minecraft']}-")
        return f"{row['minecraft']}-forge-{forge_loader}"
...
def prepare_client_install(
    ...
    key = safe_id(f"{row['loader']}-{row['minecraft']}-{row['loader_version']}")
                ...
                try:
                    ...
                    with install_log.open("a", encoding="utf-8") as log:
                        log.write(
                            ...
                            f"vanilla Minecraft {row['minecraft']}\n"
                        )
                    minecraft_launcher_lib.install.install_minecraft_version(
                        row["minecraft"], str(attempt)
                    )
                    if row["loader"] == "fabric":
                        arguments = [
                            java,
...
def prepare_server(
    ...
    with leased_installer(matrix, row, store) as installer:
        if row["loader"] == "fabric":
            ...
            return [java, "-Xms512M", "-Xmx1024M", SERVER_E2E_PROPERTY, "-jar", str(launcher), "nogui"]
...
# run always hands out the same figure.
SERVER_E2E_PROPERTY = "-Dblockpops.e2e.enabled=true"

COLLECTIONS_PATH = Path("common/src/main/resources/data/blockpops/collections")
# The in-world scenario photographs one wall per collection. The harness walks to the same
# positions (common/src/e2e/.../scenario/InWorldScenario.java), so both follow this layout:
# collections with figures, sorted by id; wall i starts at x = WALL_ORIGIN_X + i * WALL_SPACING
# on z = WALL_Z; figure k of the collection's JSON stands at column k % 9, row k // 9.
SHOWCASE_COLUMNS = 9
SHOWCASE_WALL_ORIGIN_X = -70
SHOWCASE_WALL_SPACING = 14
SHOWCASE_WALL_Z = -20
SHOWCASE_GROUND_Y = -60


def showcase_collections(root: Path) -> list[tuple[str, list[str]]]:
    """Every collection with figures, sorted by id, with its figure ids in authored order."""

    collections = []
    for path in sorted(root.glob("*.json")):
        record = json.loads(path.read_text(encoding="utf-8"))
        figures = [figure["id"] for figure in record.get("figures", [])]
        if figures:
            collections.append((record["id"], figures))
    return sorted(collections)


def showcase_function(collections: list[tuple[str, list[str]]]) -> str:
    """The function that builds every collection wall from real box blocks."""

    last_x = SHOWCASE_WALL_ORIGIN_X + (len(collections) - 1) * SHOWCASE_WALL_SPACING
    lines = [
        "# Generated by e2e/packaged_runtime.py from the packaged collections.",
        # Newer servers keep only a small spawn area loaded; setblock needs the chunks.
        f"forceload add {SHOWCASE_WALL_ORIGIN_X} {SHOWCASE_WALL_Z} "
        f"{last_x + SHOWCASE_COLUMNS} {SHOWCASE_WALL_Z}",
    ]
    for index, (collection, figures) in enumerate(collections):
        origin = SHOWCASE_WALL_ORIGIN_X + index * SHOWCASE_WALL_SPACING
        for position, figure in enumerate(figures):
            x = origin + position % SHOWCASE_COLUMNS
            y = SHOWCASE_GROUND_Y + position // SHOWCASE_COLUMNS
            lines.append(
                f"setblock {x} {y} {SHOWCASE_WALL_Z} blockpops:box_block[facing=south]"
                f'{{CollectionId:"{collection}",FigureId:"{figure}"}}'
            )
    return "\n".join(lines) + "\n"
...
def write_server_files(server: Path, port: int, template_root: Path, minecraft: str) -> None:
    ...
    # template_root is <repository>/e2e/server-template.
    showcase = showcase_function(showcase_collections(template_root.parents[1] / COLLECTIONS_PATH))
    for functions in ("function", "functions"):
        target = datapack / "data" / "blockpops_e2e" / functions / "showcase.mcfunction"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(showcase, encoding="utf-8")
...
def client_command(
    ...
    options.update(
        {
            "username": username,
            "uuid": uuid.uuid5(uuid.NAMESPACE_DNS, f"blockpops-e2e-{username}").hex,
            "token": "blockpops-e2e-offline",
            "executablePath": java,
...
def failed_marker_summary(game_dir: Path, role: str) -> str:
    ...
    try:
        report, _ = read_secure_json(
            report_path,
            label=f"{role} failed report",
            max_bytes=MAX_EVIDENCE_REPORT_BYTES,
        )
...
def inspect_screenshot(
    ...
    try:
        ...
        with Image.open(path) as image:
            ...
            if (width, height) != GUI_TEXT_REFERENCE_SIZE:
                raise RuntimeFailure(
                    f"screenshot dimensions must be the contracted "
                    f"{GUI_TEXT_REFERENCE_SIZE[0]}x{GUI_TEXT_REFERENCE_SIZE[1]}: "
...
def validate_report(game_dir: Path, row: dict[str, Any], scenario: str, role: str) -> dict[str, Any]:
    ...
    if report["schema_version"] != 1:
        raise RuntimeFailure(f"{role} report schema_version is unsupported")
    if report["contract_sha256"] != SCENARIO_CONTRACT.sha256:
        raise RuntimeFailure(
            f"{role} report scenario contract hash mismatch: "
            f"{report.get('contract_sha256')!r} != {SCENARIO_CONTRACT.sha256!r}"
        )
    if report["minecraft"] != row["minecraft"]:
        raise RuntimeFailure(f"{role} report runtime version mismatch")
    if report["role"] != role or report["scenario"] != scenario:
        raise RuntimeFailure(f"{role} report identity mismatch")
    steps = report["steps"]
    if not isinstance(steps, list):
        raise RuntimeFailure(f"{role} report steps must be an array")
    ...
    for index, raw_step in enumerate(steps):
        try:
            normalized_steps.append(
                require_object(
                    raw_step,
                    label=f"{role} report.steps[{index}]",
                    required={"id", "status", "message", "capture_id", "screenshot"},
                )
            )
        except SecureJsonError as exc:
            raise RuntimeFailure(str(exc)) from exc
    steps = normalized_steps
    actual_steps = [step["id"] for step in steps]
    ...
    for step_contract, step in zip(role_contract.steps, steps, strict=True):
        if step["status"] not in {"pass", "fail", "timeout"}:
            raise RuntimeFailure(
                f"{role}/{step_contract.id} has an invalid step status"
            )
        if (
            not isinstance(step["message"], str)
            or len(step["message"]) > 1024
            or "\x00" in step["message"]
        ):
            raise RuntimeFailure(
                f"{role}/{step_contract.id} has an invalid assertion message"
            )
        if step_contract.assertion_required and step["status"] != "pass":
            raise RuntimeFailure(
                f"{role}/{step_contract.id} required assertion did not pass"
            )
        screenshot = step["screenshot"]
        capture_required = step_contract.capture is not None
        expected_capture_id = f"{scenario}.{role}.{step_contract.id}"
        if step["capture_id"] != (expected_capture_id if capture_required else None):
            raise RuntimeFailure(
                f"{role}/{step_contract.id} semantic capture_id is missing or stale"
            )
        if capture_required:
            expected_basename = expected_capture_id + ".png"
            if screenshot != expected_basename:
                raise RuntimeFailure(
                    f"{role}/{step_contract.id} screenshot basename must be "
        ...
        if capture_required:
            ...
            if Path(screenshot).name != screenshot:
                raise RuntimeFailure(
                    f"{role}/{step_contract.id} screenshot is not a basename"
                )
            ...
            try:
                ...
                raise RuntimeFailure(
                    f"{role}/{step_contract.id} screenshot is missing: {exc}"
                ) from exc
            if (
                stat.S_ISLNK(screenshot_stat.st_mode)
                or not stat.S_ISREG(screenshot_stat.st_mode)
                or screenshot_stat.st_size <= 0
                or screenshot_stat.st_size > MAX_EVIDENCE_SCREENSHOT_BYTES
            ):
                raise RuntimeFailure(
                    f"{role}/{step_contract.id} screenshot is not a bounded regular file"
                )
    ...
    screenshots_directory = game_dir / "screenshots"
    actual_screenshots = {
        path.name
    ...
    expected_screenshots = {path.name for path in screenshot_paths.values()}
    if actual_screenshots != expected_screenshots:
        raise RuntimeFailure(
            f"{role} screenshot inventory mismatch: expected "
            f"{sorted(expected_screenshots)}, found {sorted(actual_screenshots)}"
        )
...
def validate_packaged_result(result: dict[str, Any]) -> None:
    expected_keys = {
        "schema_version",
        "artifact_node",
        "minecraft",
    ...
    if set(result) != expected_keys:
        raise RuntimeFailure(
            f"packaged result schema drift: expected {sorted(expected_keys)}, "
            f"found {sorted(result)}"
        )
    if result["schema_version"] != 1 or result["status"] not in {"pass", "fail"}:
        raise RuntimeFailure("packaged result schema/status is invalid")
    for key in ("artifact_node", "minecraft", "loader", "scenario", "contract_sha256", "profile"):
        if not isinstance(result[key], str) or not result[key] or len(result[key]) > 512:
            raise RuntimeFailure(f"packaged result {key} is invalid")
    if (
        isinstance(result["port"], bool)
        or not isinstance(result["port"], int)
        or not 1 <= result["port"] <= 65535
    ):
        raise RuntimeFailure("packaged result port is invalid")
    if (
        isinstance(result["elapsed_s"], bool)
        or not isinstance(result["elapsed_s"], (int, float))
        or not 0 <= result["elapsed_s"] <= 24 * 60 * 60
    ):
        raise RuntimeFailure("packaged result elapsed_s is invalid")
    for key in ("production_jar_sha256", "harness_jar_sha256"):
        value = result[key]
        if value is not None and (
            not isinstance(value, str) or SHA256_PATTERN.fullmatch(value) is None
        ):
            raise RuntimeFailure(f"packaged result {key} is invalid")
    if not isinstance(result["installed_blockpops"], list):
        raise RuntimeFailure("packaged result installed_blockpops must be an array")
    for index, installed in enumerate(result["installed_blockpops"]):
        if (
            not isinstance(installed, dict)
            or set(installed) != {"path", "sha256"}
            or not isinstance(installed["path"], str)
            or "\\" in installed["path"]
            or ":" in installed["path"]
            or PurePath(installed["path"]).is_absolute()
            or ".." in Path(installed["path"]).parts
            or SHA256_PATTERN.fullmatch(str(installed["sha256"])) is None
        ):
            raise RuntimeFailure(
                f"packaged result installed_blockpops[{index}] is invalid"
            )
    if not isinstance(result["reports"], dict):
        raise RuntimeFailure("packaged result reports must be an object")
    error = result["error"]
    if error is not None and (
        not isinstance(error, str) or not error or len(error) > 4096 or "\x00" in error
    ):
        raise RuntimeFailure("packaged result error is invalid")
    if result["status"] == "pass":
        if (
            result["production_jar_sha256"] is None
            or result["harness_jar_sha256"] is None
            or not result["installed_blockpops"]
            or not result["reports"]
            or error is not None
        ):
            raise RuntimeFailure("passing packaged result is incomplete")
    elif error is None:
        raise RuntimeFailure("failing packaged result must explain the failure")
...
def run_packaged_row(
    ...
    port = allocate_port()
    identity = safe_id(f"{row['artifact_node']}--{row['minecraft']}--{scenario}")
    ...
    result: dict[str, Any] = {
        ...
        "production_jar_sha256": None,
        "harness_jar_sha256": None,
        "port": port,
        "status": "fail",
        ...
        "reports": {},
        "elapsed_s": 0.0,
        "error": "packaged execution did not start",
    }
    ...
    try:
        ...
        result["production_jar_sha256"] = production_record["sha256"]
        result["harness_jar_sha256"] = harness_record["sha256"]
        stage = manifest_path.parent
        release_jar = stage / production_record["path"]
        harness_jar = stage / harness_record["path"]
        if sha256(release_jar) != production_record["sha256"]:
            raise RuntimeFailure(f"fan-in artifact hash mismatch: {release_jar}")
        if sha256(harness_jar) != harness_record["sha256"]:
        ...
        with runtime_dependencies(
            ...
            installed_blockpops: list[dict[str, str]] = []
            for game_dir in (server, *(client_directories[role] for role in roles)):
                destination = copy_verified(
                    release_jar, game_dir / "mods", production_record["sha256"]
                )
                installed_blockpops.append(
                    {
        ...
        result["installed_blockpops"] = installed_blockpops
        if (
            len(installed_blockpops) != 1 + len(roles)
            or len({item["path"] for item in installed_blockpops})
            != len(installed_blockpops)
            or any(
                item["sha256"] != production_record["sha256"]
                for item in installed_blockpops
            )
        ):
            raise RuntimeFailure(
                "installed_blockpops does not prove one exact production jar "
